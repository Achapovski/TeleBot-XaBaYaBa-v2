...
from custom_filters.filters import CategoriesStateFilter, WorkStateFilter
...
@router.message(WorkStateFilter(), Command("as"))
...
@router.message(WorkStateFilter(), Command("categories"))
async def get_categories_process(message: Message, dialog_manager: DialogManager, db_session: async_sessionmaker,
                                 msgs_to_del: list):
    await dialog_manager.start(UserCategoriesStates.get_categories, data={"db_session": db_session})


@router.message(WorkStateFilter(), Command("aliases"))
async def get_aliases_process(message: Message, dialog_manager: DialogManager, db_session: async_sessionmaker,
                              i18n: TranslatorRunner):
    pass


@router.message(WorkStateFilter(), Command("settings"))
async def change_bot_settings_process(message: Message, dialog_manager: DialogManager, db_session: async_sessionmaker,
                                      i18n: TranslatorRunner, msgs_to_del: list):
    user_settings = await SettingsDBRequests.get_params(db_session, message.from_user.id)
    data = {"user_settings": user_settings, "i18n": i18n, "language_code": user_settings.language_code,
            "currency_type": user_settings.monetary_currency, "db_session": db_session}
...
async def return_to_working_dialog_process(message: Message, dialog_manager: DialogManager,
                                           db_session: async_sessionmaker, msgs_to_del: list):
